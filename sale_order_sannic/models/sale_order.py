...
class SaleOrder(models.Model):
    ...
    def create_analytic_account(self,rec):
        dic = {
        'name': rec.name,
        'partner_id': rec.partner_id.id
        }
        self.env['account.analytic.account'].create(dic)


    @api.model
    def create(self, vals):
        rec = super(SaleOrder, self).create(vals)
        self.create_analytic_account(rec)      
        return rec
...
class SaleOrderLine(models.Model):
    ...
    cost_total = fields.Float(string="Total")
    real_purchase_price = fields.Float(string="Costo real unitario",tracking=True)
    margin_percent_real = fields.Float(
